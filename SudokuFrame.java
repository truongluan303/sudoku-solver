    ...
    private void initComponents() {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                if (count % 2 == 0) subPanels[i][j].setBackground(new Color(179, 223, 244));
                else subPanels[i][j].setBackground(new Color(252, 236, 182));
    ...
    private ActionListener action() {
        return e -> {
            ...
            if (!solved) {
                ...
                // change the message box to inform the sudoku is solved and display the solved sudoku
                if (results != null) {
                ...
                okButton.setText("OK");
                solved = true;
                myPrinter.write(inputArr, results);  // Output to the history text file
            }
            // Once the input sudoku is solved and result is displayed,
            // clicking the button will clear out the cells
            else {
                for (int i = 0; i < SIZE; i++) {
                    for (int j = 0; j < SIZE; j++) {
                        inputs[i][j].setText("");
